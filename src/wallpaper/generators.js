...
class generatorBiohazard extends ShapeGenerator {
    generate(shapeList) {
        ...
        let dt = [];
        for (let i = 0; i <= 720; i++) {
            const rad = (i * Math.PI * 1.75 / 720) - Math.PI * 157 / 180;
        ...
        dt = [];
        for (let i = 0; i <= 100; i++) {
            const rad = ((i + 10 + 120) * Math.PI / 180);
            const x = Math.sin(rad);
            const y = Math.cos(rad);
            dt.push([x * 100, y * 100]);
        }
        ...
        dt = [];
        for (let i = 0; i <= 100; i++) {
            const rad = ((i + 10 + 240) * Math.PI / 180);
            const x = Math.sin(rad);
            const y = Math.cos(rad);
            dt.push([x * 100, y * 100]);
        }
...
class generatorYinYang extends ShapeGenerator {
    generate(shapeList) {
        ...
        dt = [];
        for (let i = 0; i <= 720; i++) {
            const rad = (i * Math.PI / 360.00) + Math.PI / 2;
...
class generatorHLine extends ShapeGenerator {
    generate(shapeList) {
        ...
        const pl = new shapePointList(); pl.setData(dt2); shapeList.add(pl);
...
class generatorVLine extends ShapeGenerator {
    generate(shapeList) {
        ...
        const pl = new shapePointList(); pl.setData(dt1); shapeList.add(pl);
...
export class shapeCircle extends ShapeCreator {
    ...
    getPositionFor(perc, val) {
        ...
        let max = Math.min(this.height, this.radius);
        ...
        max = [s * this.radius + s * val * max, c * this.radius + c * val * max];
...
export class shapeHeart extends ShapeCreator {
    ...
    getPositionFor(perc, val) {
        ...
        max = [x * this.radius + (val * max * x), -y * this.radius + (val * max * -y)];
...
export class shapeLeaf extends ShapeCreator {
    ...
    getPositionFor(perc, val) {
        ...
        let x1 = (r * this.radius * 0.3 - r * max * 0.3) * Math.sin(rad + Math.PI / 2);
        let y1 = (r * this.radius * 0.3 - r * max * 0.3) * Math.cos(rad + Math.PI / 2);
        let x2 = (r * this.radius * 0.3 + r * max * 0.3) * Math.sin(rad + Math.PI / 2);
        let y2 = (r * this.radius * 0.3 + r * max * 0.3) * Math.cos(rad + Math.PI / 2);
...
export class shapeButterfly extends ShapeCreator {
    ...
    getPositionFor(perc, val) {
        ...
        let r = 9 - 0.5 * Math.sin(rad) + 2.5 * Math.sin(3 * rad) + 2 * Math.sin(5 * rad) - 1.7 * Math.sin(7 * rad)
              + 3 * Math.cos(2 * rad) - 2 * Math.cos(4 * rad) - 0.4 * Math.cos(16 * rad);
        r /= 10;
        ...
        let x1 = (r * this.radius - r * max) * Math.sin(rad + Math.PI / 2);
        let y1 = (r * this.radius - r * max) * Math.cos(rad + Math.PI / 2);
        let x2 = (r * this.radius + r * max) * Math.sin(rad + Math.PI / 2);
        let y2 = (r * this.radius + r * max) * Math.cos(rad + Math.PI / 2);
